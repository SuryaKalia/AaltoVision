...
label_file = 'NN_test_pairs.txt'

file_ = open(label_file,"r")
out_file = open("quat_trans_3.bin",'wb')

for itr in range(85000):
    line = file_.readline()
    ...
    # read data
    img1 = Image.open(os.path.join(examples_dir,'/l/Dependencies_l/Test/demon/examples' + '/' + split_line[2] + split_line[0]))
    img2 = Image.open(os.path.join(examples_dir,'/l/Dependencies_l/Test/demon/examples' + '/' + split_line[3] + split_line[1]))
    ...
    # print ("rotation in angle-axis (reduced)%s" % rotation)
    theta = math.sqrt(math.pow(rotation[0][0],2) + math.pow(rotation[0][1],2) + math.pow(rotation[0][2],2))

    imag = [((math.sin(theta/2))/theta) * rotation[0][0] , ((math.sin(theta/2))/theta) * rotation[0][1], ((math.sin(theta/2))/theta) * rotation[0][2] ]
    real = math.cos(theta/2)
    #quaternion = [real , imag[0] , imag[1] , imag[2] ]
    answer = [real , imag[0] , imag[1] , imag[2] , translation[0][0] , translation[0][1], translation[0][2]]
    # print ("quaternion: %s" % quaternion)
    # print ("norm check: %s" % np.sqrt(np.dot(quaternion,quaternion)))
    # print ("translation %s" % translation[0])
    # print ("saved file number %d" % itr)

    #line_string = (' '.join(str(e) for e in quaternion)) + " " + (' '.join(str(e) for e in translation[0]) + "\n")


    # out_file.write(' '.join(str(e) for e in quaternion))
    # out_file.write(" ")
    # out_file.write(' '.join(str(e) for e in translation[0]))
    # out_file.write("\n")

    a = np.array(answer,'float32')
    a.tofile(out_file)

    #out_file.write((float_to_binary(3.45)).encode())
    #out_file.write(bytes(line_string, 'UTF-8'))
